CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_report_port_outward`(IN `vessel` VARCHAR(100), IN `dispatch` VARCHAR(100), IN `frmdt` VARCHAR(50), IN `todt` VARCHAR(50), IN `grade` VARCHAR(100), OUT `message` VARCHAR(100))
BEGIN
...
and po.grade = CASE WHEN grade = 'ALL' THEN po.grade ELSE grade END;
