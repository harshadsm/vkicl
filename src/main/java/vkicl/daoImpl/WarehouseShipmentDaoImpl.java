...
public class WarehouseShipmentDaoImpl extends BaseDaoImpl {
	...
	public void updateWarehouseInwardFlag(WarehouseInwardRecordVO postDataContainer, UserInfoVO userInfo)
		...
		try {
			...
			// java.sql.Date vehicleDate =
			// Converter.dateToSqlDate(Converter.stringToDate(postDataContainer.getVehicleDate(),
			// Constants.Apps.DATE_FORMAT));

			query = "UPDATE port_outward SET " + " warehouse_inward_flag =1 WHERE port_out_id="
					+ postDataContainer.getPortOutwardId();
